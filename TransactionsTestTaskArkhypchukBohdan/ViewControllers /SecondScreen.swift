...
final class SecondScreen: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setUpAmountLabel() // Initial setup of the UI for the ViewController
        setUpAmountField()
        setUpCategoryPicker()
        setUpAddButton()
    }
...
extension SecondScreen {
    ...
    func setUpAmountField() {
        ...
        NSLayoutConstraint.activate([
            ...
            amountField.topAnchor.constraint(equalTo: amountLabel.bottomAnchor,
                                             constant: C.Constraints.yDistanceButtons)
        ])
    }
    ...
    func setUpCategoryPicker() {
        ...
        NSLayoutConstraint.activate([
            ...
            categoryPicker.topAnchor.constraint(equalTo: amountField.bottomAnchor,
                                                constant: C.Constraints.yDistanceButtons)
        ])
    }
    ...
    func setUpAddButton() {
        ...
        NSLayoutConstraint.activate([
            ...
            addButton.topAnchor.constraint(equalTo: categoryPicker.bottomAnchor,
                                           constant: C.Constraints.yDistanceButtons)
        ])
    }
    ...
    @objc func addTransaction() {
        ...
        if let amount = self.amount {
            if currentBalance >= amount {  // Checking whether the user can perform transaction based on their current balance
                CoreDataService.shared.write { // Creating new transaction
                    ...
                    balance.first?.amount = currentBalance.rounded(toPlaces: C.roundDecimal) -
                                            amount.rounded(toPlaces: C.roundDecimal) // Deducting the amount of the transaction from the balance accordingly
                    navigationController?.popToRootViewController(animated: true)
                }
                delegate?.addTransaction()
            } else {
                let notEnoughMoney = UIAlertController(title: "Oops",
                                                       message: C.Strings.wrongAmountMessage,
                                                       preferredStyle: .alert) // If there are not enough money on the balance showing this error
                present(notEnoughMoney, animated: true, completion: {
                    notEnoughMoney.view.superview?.isUserInteractionEnabled = true
                    notEnoughMoney.view.superview?.addGestureRecognizer(UITapGestureRecognizer(
                        target: self,
